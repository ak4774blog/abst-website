import AnimatedPage from '../components/AnimatedPage';

export default function Register() {
  return (
    <AnimatedPage>
      <div className="max-w-2xl mx-auto px-4 py-12 sm:px-6 lg:px-8 text-center">
        <h1 className="text-4xl font-extrabold tracking-tight text-white">Register for ABST!</h1>
        <p className="mt-4 text-lg text-slate-400">
          You can find all the information about the competition on the Competition Information tab.
        </p>
        <div className="mt-8 bg-slate-800 p-10 rounded-lg border border-slate-700">
          <h2 className="text-2xl font-bold text-slate-100">Registration Form Coming Soon</h2>
          <p className="text-slate-400 mt-2">Please check back closer to the event date.</p>
        </div>
      </div>
    </AnimatedPage>
  );
}
